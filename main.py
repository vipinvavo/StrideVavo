...
st.set_page_config(
    page_title="StrideVavo — AI Threat Modeling",
    page_icon="🛡️",
    layout="wide",
...
html,body,[class*="css"]{font-family:'JetBrains Mono','Fira Code','Courier New',monospace!important}

/* Hero */
.hero{padding:2rem 0 1.5rem}
.hero h1{font-size:2.4rem!important;font-weight:800!important;color:#00ff41!important;letter-spacing:-.02em;line-height:1.15;margin:0!important;text-shadow:0 0 20px rgba(0,255,65,.15)}
.hero .prompt{color:#555;font-size:1rem;margin:0 0 .3rem 0}
.hero p{color:#666;font-size:.95rem;line-height:1.6;margin:.6rem 0 0}

/* Tabs */
.stTabs [data-baseweb="tab-list"]{gap:0;background:#111;padding:4px;border-radius:6px;border:1px solid #1a1a1a}
.stTabs [data-baseweb="tab"]{padding:10px 20px;font-weight:600;font-size:.84rem;color:#555;border:none;border-radius:4px;transition:all .15s}
.stTabs [data-baseweb="tab"]:hover{color:#00ff41}
.stTabs [aria-selected="true"]{color:#0a0a0a!important;background:#00ff41!important;font-weight:700;box-shadow:0 0 12px rgba(0,255,65,.2)}

/* Buttons */
.stButton>button{background:#00ff41!important;color:#0a0a0a!important;border-radius:6px!important;font-weight:700!important;padding:.5rem 1.6rem!important;border:none!important;font-size:.85rem!important;font-family:'JetBrains Mono',monospace!important;text-transform:uppercase!important;letter-spacing:.05em!important;transition:all .15s!important;box-shadow:0 0 10px rgba(0,255,65,.15)!important}
.stButton>button:hover{box-shadow:0 0 25px rgba(0,255,65,.35)!important;transform:translateY(-1px)!important}
.stDownloadButton>button{background:transparent!important;color:#00ff41!important;border:1px solid #1a3a1a!important;border-radius:6px!important;font-weight:600!important;font-family:'JetBrains Mono',monospace!important;transition:all .15s!important}
.stDownloadButton>button:hover{background:#0a1a0a!important;border-color:#00ff41!important;box-shadow:0 0 10px rgba(0,255,65,.1)!important}

/* Sidebar */
section[data-testid="stSidebar"]{background:#080808!important;border-right:1px solid #1a1a1a!important}
.sb-brand{padding:.5rem 0 .15rem}
.sb-brand h2{color:#00ff41!important;font-weight:700!important;font-size:1rem!important;margin:0!important;text-shadow:0 0 8px rgba(0,255,65,.2)}
.sb-brand .sub{color:#444;font-size:.7rem;font-weight:500;margin-top:3px;display:block}
.sb-brand .sub b{color:#00ff41;font-weight:700}
.sb-div{border:none;height:1px;background:#1a1a1a;margin:.75rem 0}

/* Inputs */
.stTextInput>div>div>input,.stTextArea>div>div>textarea{border-radius:6px!important;border:1px solid #1a1a1a!important;background:#111!important;color:#c0c0c0!important;font-family:'JetBrains Mono',monospace!important;transition:all .15s!important}
.stTextInput>div>div>input:focus,.stTextArea>div>div>textarea:focus{border-color:#00ff41!important;box-shadow:0 0 0 2px rgba(0,255,65,.1)!important}
.stSelectbox>div>div{border-radius:6px!important}
.stAlert{border-radius:6px!important}

/* Footer */
.ft{text-align:center;padding:2.5rem 0 1rem;margin-top:3rem;color:#333;font-size:.75rem}
.ft b{color:#00ff41;text-shadow:0 0 6px rgba(0,255,65,.2)}
...
st.sidebar.markdown(
    '<div class="sb-brand"><h2>&gt;_ StrideVavo</h2></div><hr class="sb-div">',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.caption("Your data is never stored. Everything is cleared when you close this tab.")


# ------------------ Main App UI ------------------ #

st.markdown(
    '<div class="hero"><p class="prompt">$ ./stridevavo --analyze</p><h1>StrideVavo</h1><p>AI-powered STRIDE threat modeling. Describe your target. Get results.</p></div>',
    unsafe_allow_html=True,
)

tab1, tab2, tab3 = st.tabs(["Threats", "Attack Tree", "Deep Analysis"])
...
# ------------------ Deep Analysis (Mitigations + DREAD + Test Cases) ------------------ #

with tab3:
    st.markdown("Generate mitigations, risk scores, and security test cases from your threat model.")
    da1, da2, da3 = st.columns(3)
    with da1:
        mitigations_submit_button = st.button(label="Mitigations")
    with da2:
        dread_assessment_submit_button = st.button(label="DREAD Scores")
    with da3:
        test_cases_submit_button = st.button(label="Test Cases")
...
st.markdown('<div class="ft">root@stridevavo:~# built by Vipin // product_security @ bitvavo</div>', unsafe_allow_html=True)
